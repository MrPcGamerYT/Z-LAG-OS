...
$cursorPaths = @{
    ...
    "No"            = "$DestinationFolder\unavailiable.cur"
...
# 5. Also set current user's cursor
...
# 6. Keep Control Panel > Mouse > Pointers tab visible
# NoChangingMousePointers hides that Pointers tab.
$policyPaths = @(
...
foreach ($policyPath in $policyPaths) {
    if (Test-Path $policyPath) {
        Remove-ItemProperty -Path $policyPath -Name "NoChangingMousePointers" -ErrorAction SilentlyContinue
        Remove-ItemProperty -Path $policyPath -Name "PreventChangingTheme" -ErrorAction SilentlyContinue
    }
}
...
Write-Host "ElegantDark cursor applied for all users. Control Panel Mouse Pointers tab remains available." -ForegroundColor Cyan
