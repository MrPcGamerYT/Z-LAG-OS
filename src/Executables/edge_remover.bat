@echo off
title Edge Remover - Ultimate Edition

:: ============================================================
:: CREDIT: Original Edge removal by ShadowWhisperer
:: (https://github.com/ShadowWhisperer/Remove-MS-Edge)
:: Modified and enhanced for Z-LAG OS
:: ============================================================
...
:: ============================================================
:: EDGE SHORTCUT REMOVER (Desktop, Start Menu, AND TASKBAR)
:: ============================================================
echo - Removing Edge shortcuts from all users (Desktop, Start Menu, Taskbar)

:: Remove from all user desktops
for /f "skip=1 tokens=2*" %%a in ('reg query "HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\ProfileList" /s 2^>nul ^| findstr /i "ProfileImagePath"') do (
    set "userpath=%%b"
    if exist "!userpath!\Desktop\" (
        if exist "!userpath!\Desktop\edge.lnk" del /f /q "!userpath!\Desktop\edge.lnk" >nul 2>&1
        if exist "!userpath!\Desktop\Microsoft Edge.lnk" del /f /q "!userpath!\Desktop\Microsoft Edge.lnk" >nul 2>&1
    )
)

:: Remove from Public Desktop
if exist "%Public%\Desktop\edge.lnk" del /f /q "%Public%\Desktop\edge.lnk" >nul 2>&1
if exist "%Public%\Desktop\Microsoft Edge.lnk" del /f /q "%Public%\Desktop\Microsoft Edge.lnk" >nul 2>&1

:: Remove from Start Menu
if exist "%ProgramData%\Microsoft\Windows\Start Menu\Programs\edge.lnk" del /f /q "%ProgramData%\Microsoft\Windows\Start Menu\Programs\edge.lnk" >nul 2>&1
...
:: ============================================================
:: REMOVE EDGE FROM TASKBAR (Pinned icons)
:: ============================================================
echo - Removing Edge from Taskbar

:: Remove from current user's Taskbar
powershell -Command "$taskbar = Join-Path $env:APPDATA 'Microsoft\Internet Explorer\Quick Launch\User Pinned\TaskBar'; if (Test-Path $taskbar) { Get-ChildItem $taskbar -Filter '*edge*.lnk' | ForEach-Object { Remove-Item $_.FullName -Force -ErrorAction SilentlyContinue } }"

:: Remove from all user Taskbars
for /f "skip=1 tokens=2*" %%a in ('reg query "HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\ProfileList" /s 2^>nul ^| findstr /i "ProfileImagePath"') do (
    set "userpath=%%b"
    set "taskbarpath=!userpath!\AppData\Roaming\Microsoft\Internet Explorer\Quick Launch\User Pinned\TaskBar"
    if exist "!taskbarpath!\" (
        if exist "!taskbarpath!\edge.lnk" del /f /q "!taskbarpath!\edge.lnk" >nul 2>&1
        if exist "!taskbarpath!\Microsoft Edge.lnk" del /f /q "!taskbarpath!\Microsoft Edge.lnk" >nul 2>&1
    )
)

:: Remove Edge from Taskbar via registry (unpins it)
powershell -Command "try { $app = Get-StartApps | Where-Object { $_.Name -like '*edge*' -or $_.AppId -like '*edge*' }; if ($app) { $app | ForEach-Object { UnpinFromTaskbar $_.AppId -ErrorAction SilentlyContinue } } } catch { }"

echo - Edge shortcut removal completed (including Taskbar)
...
echo.
echo ============================================================
echo  Edge Removal by ShadowWhisperer
echo  Enhanced for Z-LAG OS
echo ============================================================
echo.
