...
namespace ZLagOS
{
    ...
    public static class OptiServicesHost
    {
        ...
        {
            if (args != null && Array.IndexOf(args, "--self-test") >= 0)
            {
                Application.EnableVisualStyles();
                Application.SetCompatibleTextRenderingDefault(false);
                using (var form = new WelcomeForm()) form.CreateControl();
                return;
            }
...
function Test-ZLagWelcomeExecutable {
    param([string]$Path, [int]$Runs = 1)
    for ($attempt = 1; $attempt -le $Runs; $attempt++) {
        try {
            $process = Start-Process -FilePath $Path -ArgumentList '--self-test' -WindowStyle Hidden -PassThru -ErrorAction Stop
            if (-not $process.WaitForExit(10000)) {
                Stop-Process -Id $process.Id -Force -ErrorAction SilentlyContinue
                throw ('self-test ' + $attempt + ' timed out')
            }
            if ($process.ExitCode -ne 0) { throw ('self-test ' + $attempt + ' returned ' + $process.ExitCode) }
        } catch {
            throw ('Executable self-test failed: ' + $_.Exception.Message)
        }
    }
    return $true
}
...
try {
    ...
    [void](Test-ZLagWelcomeExecutable -Path $tempExe -Runs 3)
    ...
    [void](Test-ZLagWelcomeExecutable -Path $welcomeExe -Runs 1)
